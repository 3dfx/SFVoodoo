		...
		if (sfv_file.equals("")) {
			...
			if (file.exists()) {
				if (file.isFile()) {
					...
					//sfv_file = path + "no_name_given.sfv";
				}
				else {
				    if (files.length == 1) {
					...
					//sfv_file = file.toString() + "no_name_given.sfv";
				}
			}
			else {
			    System.err.print("ERROR:\t\"" + file + "\" not found!");
				return -1;
//				throw new FileNotFoundException(sfv_file);
			}
		}
...
//			throw new FileNotFoundException(sfv_file);
    ...
    private void calculate(String path, BufferedWriter out) throws FileNotFoundException, IOException {
