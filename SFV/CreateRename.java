		    ...
		    if (file.exists()) {
				if (file.isDirectory()) {
					...
					for (j = 0; j < nfiles.length; j++) {
						//nfiles[j] = path + (file.isFile() ? File.separator : "") + nfiles[j];
					    //nfiles[j] = path + (path.endsWith(File.separator) ? "" : File.separator) + nfiles[j];
					    nfiles[j] = path + File.separator + nfiles[j];
					    file = new File(nfiles[j]);
					    if (file.isDirectory())
					        calculate(file.toString() + File.separator);
					        //calculate(file.toString() + (file.toString().endsWith(File.separator) ? "" : File.separator), out);
					    else if(!reached) {
					        i = j;
					        reached = true;
					    }
