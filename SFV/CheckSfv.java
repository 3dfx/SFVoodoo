            ...
            while ((tmp = in.readLine()) != null) {
            	if (!tmp.equalsIgnoreCase("") && tmp.charAt(0) != ';') {
            		...
            		if (cur_file.exists()) {
            			...
            			/*if (res.equalsIgnoreCase(checked_crc)) {
            				echo("\t\t[" + res + "]\t\n" + (i > 9 ? "" : "0") + i + ".\t\tCRC OK\t\t" + getFileName(file_name) + "\n");
            				crc_ok++;
            			}
            			else {
            				echo("\t\t[" + res + " != " + checked_crc + "]\t\n" + (i > 9 ? "" : "0") + i + ".\t\tCRC FAIL\t" + getFileName(file_name) + "\n");
            				crc_fail++;
            			}*/
            		} // is (isFile)
            		else {
            		    echo("\t\tFnF\n");
            			//echo((i > 9 ? "" : "0") + i + ".\t\tFile !Found\t" + getFileName(cur_file.getAbsoluteFile().toString()) + "\n");
            			crc_miss++;
            		}
